    ...
    public void ConstructGraph() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(filename))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                logger.info(repoName.replace("\"", ""));
                String inputFile = Paths.get(this.srcDir, "dicth_" + repoName.replace("\"", "").replace("/", "__")).toString();
                String outputFile = Paths.get(this.srcDir, "graph_" + repoName.replace("\"", "").replace("/", "__")).toString();
                try (BufferedReader inputFileReader = new BufferedReader(new FileReader(inputFile));
                    ...
                    while ((inputLine = inputFileReader.readLine()) != null) {
                        ...
                        String firstElement = inputValues[0].trim();
                        if (!firstElement.isEmpty() && !firstElement.startsWith("1")) {
                            String outputLine = "1#" + firstElement;
    ...
    public void ConstructDicth() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(filename))) {
            ...
            while ((line = reader.readLine()) != null) {
                String[] values = line.split(",");
                String repoName = values[0];
                logger.info("repoo " +  repoName.replace("\"", ""));
               
                String outputFile = Paths.get(this.srcDir, "dicth_" + repoName.replace("\"", "").replace("/", "__")).toString();
                try (BufferedWriter writer = new BufferedWriter(new FileWriter(outputFile))) {
                    writer.write(1 + "\t" + repoName.replace("\"", ""));
                    writer.newLine();
                   
                    int lineNumber = 2;
                    for (String z : Arrays.copyOfRange(values, 1, values.length)) {
                        String outputLine = lineNumber + "\t" + "#DEP#" + z.trim().replace("\"", "");
                        try {
                            writer.write(outputLine);
                            writer.newLine();
                            lineNumber++;
                        ...
                        eASEOutput.put(repoName, outputLine);
                    }
                } catch (IOException e) {
                    e.printStackTrace();
                }
