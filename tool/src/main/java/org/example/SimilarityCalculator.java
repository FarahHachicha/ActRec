...
public class SimilarityCalculator {
     ...
     * using Cosine Similarity with Weight
     */
    public boolean hasMissingLibrary(Set<String> trainingLibs, Set<String> testingLibs) {
        for (String library : trainingLibs) {
            if (!testingLibs.contains(library)) {
                // La librairie du trainingLibs n'existe pas dans le testingLibs
                return true;
            }
        }
        // Toutes les librairies du trainingLibs existent dans le testingLibs
        return false;
    }
        ...
        for (Integer keyTesting : keyTestingProjects) {
            try {
                ...
                for (Integer keyTraining : keyTrainingProjects) {
                    ...
                    /* read all libraries for each project */
                    trainingLibs = reader.getLibraries(trainingDictFilename);
                    boolean hasMissingLib = hasMissingLibrary(trainingLibs, testingLibs);
                //  logger.info("avant");
                    if (hasMissingLib){
                //  logger.info("trainingLibs"+trainingLibs);
                //  logger.info("testingLibs"+testingLibs);
                    ...
                    for (int i = 0; i < size; i++) {
                        ...
                        if (testingLibs.contains(lib)) {
                            if (combinedDictionary.containsKey(lib)) {
